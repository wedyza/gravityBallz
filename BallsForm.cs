...
namespace GravityBalls
{
    public class BallsForm : Form
    {
        ...
        public BallsForm() : base()
        {
            this.KeyPreview = true;
            this.KeyPress += new KeyPressEventHandler(RefreshBallFalling);
        }

        void RefreshBallFalling(object sender, KeyPressEventArgs e)
        {
            world.BallX = world.WorldHeight / 2;
            world.BallY = world.BallRadius;
            world.velY = 0;
            world.velX = 0;

            MessageBox.Show($"Ball has returned on ({world.WorldHeight / 2}, {world.BallRadius}) coordinates.");
        ...
        {
            base.OnMouseMove(e);
            Text = string.Format("Cursor ({0}, {1}, {2}, {3})", e.X, e.Y, world.BallX, world.BallY);
        }
    }
}